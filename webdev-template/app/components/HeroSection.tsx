			...
			<div className="relative z-10 max-w-5xl mx-auto px-4">
				<div className="text-center space-y-6">
					...
					<div className="relative inline-block px-4 sm:px-0">
						<div className="absolute -inset-1 bg-gradient-to-r from-yellow-500 via-pink-500 to-indigo-500 rounded-2xl blur-xl opacity-75"></div>
					...
					<div className="space-y-4 sm:space-y-6 mt-6 sm:mt-8">
						<div className="flex flex-col items-center gap-4">
							<div className="flex items-center gap-3">
								<span className="h-[1px] w-8 sm:w-12 bg-pink-500"></span>
								<h2 className="text-xl sm:text-2xl font-light tracking-wide text-pink-400">Creative Coder & Game Developer</h2>
								<span className="h-[1px] w-8 sm:w-12 bg-pink-500"></span>
							</div>
							<p className="text-base sm:text-lg text-gray-400 max-w-2xl px-4 sm:px-0">
								From shaders to interactive storytelling, I bring ideas to life through code and art
							</p>
						</div>

						{/* Tech Stack Pills */}
						<div className="flex flex-wrap justify-center gap-2 sm:gap-3 mt-6 sm:mt-8 px-4 sm:px-0">
							{['Unity', 'Unreal', 'WebGL', 'JavaScript', 'C++'].map((tech) => (
								<span
									key={tech}
									className="px-3 sm:px-4 py-1 sm:py-1.5 bg-[#1A1F2B] rounded-full text-sm font-medium text-gray-300 border border-[#2D333B] hover:border-yellow-500/50 transition-colors"
								>
									{tech}
								</span>
							))}
					...
					<div className="flex flex-col sm:flex-row justify-center gap-3 sm:gap-4 mt-8 sm:mt-12 px-4 sm:px-0">
						<a href="#work" className="group relative inline-flex items-center justify-center">
							<div className="absolute -inset-0.5 bg-gradient-to-r from-pink-500 to-yellow-500 rounded-full blur opacity-75 group-hover:opacity-100 transition"></div>
					...
					<div className="flex justify-center gap-4 sm:gap-6 mt-8 sm:mt-12">
						{[
							...
							{ name: 'Instagram', icon: '💬', href: 'https://www.instagram.com/c_hodoroga?igsh=eWg3d25tdnhqN2Vh' },
							{ name: 'Blog', icon: '📜', href: '#' },
						].map((social) => (
